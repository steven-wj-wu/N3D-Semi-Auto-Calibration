...
public:
    ...
    //tmp save current image  for xoff_lens calibration
        Mat image_1;
        Mat image_2;
        Mat  x_off_filter;
        Mat binary_img;
    ...
    // ? Basic Image Attributes
        ...
        int* get_ref_rgb(char ref_type) {
            if (ref_type == 'R' || ref_type == 'r') {
                int* rgb = reference[R].rgb;
                return rgb;
            }

            else if (ref_type == 'G' || ref_type == 'g') {
                int* rgb = reference[G].rgb;
                return rgb;
            }
        }
        //get ref HSV value
        int* get_ref_hsv(char ref_type) {
            if (ref_type == 'R' || ref_type == 'r') {
                int* hsv = reference[R].hsv;
                return hsv;
            }

            else if (ref_type == 'G' || ref_type == 'g') {
                int* hsv = reference[G].hsv;
                return hsv;
            }
        }
        //Check if Ref Color is set
        bool is_ref_set() {
