...
#define COLOR_DISTANCE_THRESHOLD 0.35
...
void MainWindow::fpga_is_running(){
    while(1){
        if(fpga_eye_tracking.x!=-1){
            ...
            ui->fpga_status->setText(QString::fromLocal8Bit("PC 校正模組:: 斷線"));
            break;
        }
        Sleep(1000);
    }
...
void MainWindow::load_parameters(){


    if(ui->hcp_adjust_ini->text().isEmpty()&&ui->slant_adjust_ini->text().isEmpty()){
        ui->page_2_warning->setText("所有校正參數需給值");
        return;
    }else{
        ...
        {
            ui->page_2_warning->setText("所有面板參數需給值");
            return;
        }
        else{
...
void MainWindow::to_hcp_slant_page(){
    ...
    if(calibrate_mode==1){
        AUO3D_imagePath_LR("./GREEN.bmp","./RED.bmp");
    }else if(calibrate_mode==0){
        Second_Monitor_mode(R_G);
    }
...
void MainWindow::calibrate_hcp(int step){
   ...
   //ref green
       ...
       calibrate_timer->setInterval(1300);
       calibrate_timer->setSingleShot(true);
       calibrate_timer->start();
   }
   //color contrast
        ...
        int int_color = color_constrat*1000;

        if (int_color <  COLOR_DISTANCE_THRESHOLD*1000) { //if value is not OK adjust the pitch
            ...
            if (my_hcp_slant.check_times < 3) { //check 3 times if the value is OK
   ...
   //slant
        ...
        double rg_ratio;
        int int_rg_ratio;
       if (my_hcp_slant.slant<my_hcp_slant.max_slant && my_hcp_slant.slant>my_hcp_slant.min_slant) {
            tmp_frame = single.current_frame.clone();
            res = Calibrate.set_image_by_camera(tmp_frame);
            rg_ratio = Calibrate.get_RG_Ratio();
            int_rg_ratio = rg_ratio*1000;
            ui->rg_ratio->setText( QString::fromStdString(to_string(rg_ratio)));
           }

       if (my_hcp_slant.slant<my_hcp_slant.max_slant && my_hcp_slant.slant>my_hcp_slant.min_slant&&int_rg_ratio < (int)my_hcp_slant.last_rg_ratio*1000) { //if  rg ratio better than last one->  adjust next value
           my_hcp_slant.is_trend = 1;
           ...
           calibrate_timer->start();
               ...
               if (my_hcp_slant.check_times < 2 &&my_hcp_slant.slant<my_hcp_slant.max_slant && my_hcp_slant.slant>my_hcp_slant.min_slant) { //if rg ratio get worse with same value at certain times
                   my_hcp_slant.check_times++;
                   hcp_slant_step->setMapping(calibrate_timer, 4);
                   calibrate_timer->start();
               }
   ...
   //hcp
       ...
       int int_rg_ratio;
       //get rg_ratio
       if (my_hcp_slant.hcp > my_hcp_slant.min_hcp && my_hcp_slant.hcp < my_hcp_slant.max_hcp) {
           tmp_frame = single.current_frame.clone();
           res = Calibrate.set_image_by_camera(tmp_frame);
           rg_ratio = Calibrate.get_RG_Ratio();
           int_rg_ratio = rg_ratio*1000;
       ...
       if (my_hcp_slant.hcp > my_hcp_slant.min_hcp && my_hcp_slant.hcp < my_hcp_slant.max_hcp&& int_rg_ratio < (int)my_hcp_slant.last_rg_ratio*1000) {
           my_hcp_slant.is_trend = 1;
           my_hcp_slant.last_rg_ratio = rg_ratio;
           //continue adjust hcp
           ui->hcp->setText(QString::fromStdString(to_string(my_hcp_slant.hcp)));
          ...
          calibrate_timer->start();
           ...
           if ( my_hcp_slant.check_times < 2 &&  my_hcp_slant.hcp >  my_hcp_slant.min_hcp && my_hcp_slant.hcp < my_hcp_slant.max_hcp) {
               my_hcp_slant.check_times++;
               hcp_slant_step->setMapping(calibrate_timer, 5);
               calibrate_timer->start();
           }
        ...
        Rect tmp_rect;
            ...
            if (!dual_1.current_frame.empty() && !dual_2.current_frame.empty()) {
                 ...
                 if(camera_num==1){
                    ...
                    cv::threshold(mask, mask, 80, 255, cv::THRESH_BINARY); // ? binary image
    ...
    if(step==0){
      ...
      if(dual_1.side==1){
       ...
       Calibrate.set_image_by_dual_camera(tmp_dual_1(dual_1.bounding_rect), tmp_dual_2(dual_2.bounding_rect));
        ...
        my_view_zone.rg_ratio_threshold_left =  (rg_ratio_left -  my_view_zone.rg_ratio_threshold_left ) *0.25;
        my_view_zone.rg_ratio_threshold_right =  (rg_ratio_right -  my_view_zone.rg_ratio_threshold_right) *0.25;
       ...
       ui->Calibration_Function_Box->setEnabled(false);
        ...
        if(calibrate_mode==0){
            ...
            if(my_vd_it.current_stage==0){
                ...
                my_vd_it.E1 = parameters.E1;
                my_vd_it.E2 = parameters.E2;
...
void MainWindow::move_add(){
    ...
    if(my_vd_it.current_stage<3){
             ...
             if(calibrate_mode==0){
              ...
              my_vd_it.add_move_length =  my_vd_it.add_move_length + 0.01;
...
void MainWindow::move_dec(){
    ...
    if(my_vd_it.current_stage<3){
             ...
             my_vd_it.add_move_length =  my_vd_it.add_move_length + 0.01;
             my_vd_it.tmp_C =  parameters.E1 + -1*my_vd_it.add_move_length;
             set_par();
             if(calibrate_mode==0){
                  AUO3D_FPGA_SendData(PanelData::E1,&my_vd_it.tmp_C);
             }else if(calibrate_mode==1){
             ...
             my_vd_it.add_move_length =  my_vd_it.add_move_length + 0.01;
             my_vd_it.tmp_C =   parameters.E2 +-1* my_vd_it.add_move_length;
             set_par();
             if(calibrate_mode==0){
                  AUO3D_FPGA_SendData(PanelData::E2,&my_vd_it.tmp_C);
             }else if(calibrate_mode==1){
...
void MainWindow::set_par(){
    ...
    if(my_vd_it.current_stage==0){
        ...
        if (!my_vd_it.single_search) {
              ...
              my_vd_it.E1 = (my_vd_it.single_search_is_add) ?  parameters.E1-((my_vd_it.add_move_length+my_vd_it.dec_move_length) / 2) : parameters.E1-(-1 * (my_vd_it.dec_move_length + my_vd_it.add_move_length) / 2);
              ui->E1->setText(QString::number(my_vd_it.E1));
        }
        ...
        if (!my_vd_it.single_search) {
              ...
              my_vd_it.E2 = (my_vd_it.single_search_is_add) ? parameters.E2-((my_vd_it.add_move_length+my_vd_it.dec_move_length) / 2) : parameters.E2-(-1 * (my_vd_it.dec_move_length + my_vd_it.add_move_length) / 2);
              ui->E2->setText(QString::number(my_vd_it.E2));
...
void MainWindow::vd_id_stage(){
    if(my_vd_it.current_stage==0){
        ...
        ui->eye_pos_x->setText(QString::number(0));
        ui->eye_pos_y->setText(QString::number(0));
        ui->eye_pos_z->setText(QString::number( VD ));
        ...
        my_vd_it.current_vd = VD;
        ui->eye_pos_x->setText(QString::number(X));
        ui->eye_pos_y->setText(QString::number(0));
        ui->eye_pos_z->setText(QString::number( VD ));

        QPixmap camera_position = load_image("./4.png",ui->vd_pos->width(),ui->vd_pos->height());
        ui->vd_pos->setPixmap(camera_position);
        ...
        //E1 to E2
        double X =  -1*0.36396471 * (parameters.OVD+parameters.vd_far)/2;
        double VD = (parameters.OVD+parameters.vd_far)/2;
        ...
        QPixmap camera_position = load_image("./6.png",ui->vd_pos->width(),ui->vd_pos->height());
        ui->vd_pos->setPixmap(camera_position);
...
void MainWindow::eye_tracking_ui(){
    ...
    float vd=0;
    if(calibrate_mode==0){
        x = fpga_eye_tracking.x+parameters.cam_oft_x;
        y=fpga_eye_tracking.y+parameters.cam_oft_y;
        vd = fpga_eye_tracking.z+parameters.cam_oft_z;


    }else if(calibrate_mode==1){
        x = pc_eye_tracking.x+parameters.cam_oft_x;
        y= pc_eye_tracking.y+parameters.cam_oft_y;
        vd = pc_eye_tracking.z+parameters.cam_oft_z;
